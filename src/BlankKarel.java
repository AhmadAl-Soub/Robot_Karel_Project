...
public class BlankKarel extends SuperKarel {
	...
	private int[] getDivideInfo(int dimension , int numOfPieces){
		...
		int numberAvailableUnits = dimension- numOfPieces + 1;//7
		while (true){
			if (numberAvailableUnits % numOfPieces == 0){// [1] 7 % 4 != 0 [2] 6 % 4 != 0 [3] 5 % 4 != 0 [4] 4 % 4 == 0
				numberOfUnitsInEachArea = numberAvailableUnits / numOfPieces; // 4 / 4 = 1
				break;
			}
			numberAvailableUnits--; //4
			numOfExtraBeeper++;//3
		}
		int[] array = {numberOfUnitsInEachArea, numOfExtraBeeper, numOfPieces}; //{1, 3, 4}
		return array;
	}									// 1					3				4					10
	...
	private void divide1D(int x){
		int[] array ;
		if ((x-3) / 4 >= 1){
			array = getDivideInfo(x, 4); // {1, 3, 4}
			divideAline(array[0], array[1], array[2], x); // dividALine(1, 3, 4, 10)
		}
		else if ((x-2) / 3 >= 1){
